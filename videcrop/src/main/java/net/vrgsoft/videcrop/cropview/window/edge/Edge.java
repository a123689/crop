...
public enum Edge {
    ...
    private static float adjustLeft(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (x - (float)imageRect.left < imageSnapRadius) {
            ...
            if (x >= RIGHT.getCoordinate() - 200.0F) {
                resultXHoriz = RIGHT.getCoordinate() - 200.0F;
            }

            if ((RIGHT.getCoordinate() - x) / aspectRatio <= 200.0F) {
                resultXVert = RIGHT.getCoordinate() - 200.0F * aspectRatio;
            }
    ...
    private static float adjustRight(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if ((float)imageRect.right - x < imageSnapRadius) {
            ...
            if (x <= LEFT.getCoordinate() + 200.0F) {
                resultXHoriz = LEFT.getCoordinate() + 200.0F;
            }

            if ((x - LEFT.getCoordinate()) / aspectRatio <= 200.0F) {
                resultXVert = LEFT.getCoordinate() + 200.0F * aspectRatio;
            }
    ...
    private static float adjustTop(float y, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (y - (float)imageRect.top < imageSnapRadius) {
            ...
            if (y >= BOTTOM.getCoordinate() - 200.0F) {
                resultYHoriz = BOTTOM.getCoordinate() - 200.0F;
            }

            if ((BOTTOM.getCoordinate() - y) * aspectRatio <= 200.0F) {
                resultYVert = BOTTOM.getCoordinate() - 200.0F / aspectRatio;
            }
    ...
    private static float adjustBottom(float y, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if ((float)imageRect.bottom - y < imageSnapRadius) {
            ...
            if (y <= TOP.getCoordinate() + 200.0F) {
                resultYVert = TOP.getCoordinate() + 200.0F;
            }

            if ((y - TOP.getCoordinate()) * aspectRatio <= 200.0F) {
                resultYHoriz = TOP.getCoordinate() + 200.0F / aspectRatio;
            }
